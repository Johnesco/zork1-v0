@@ verbs.zil — Generic Verb Handlers for the Zork Trilogy
@@ Shared verb-action routines used by all three Zork games, written by
@@ SEM (Steve Meretzky) starting 7/25/83. Contains implementations for
@@ standard player actions: inventory, movement, object manipulation,
@@ combat, look/examine, save/restore, and miscellaneous interactions.
@@
...
@@ --- Game commands: verbosity settings, inventory, quit/restart/save/restore,
@@ --- script/unscript, version info, verify, command-file, random, record
...
@@ --- Real verb functions: the main alphabetical catalog of player action handlers.
@@ --- Each V-* routine is the default handler for a parser verb; PRE-* routines
@@ --- run before the action and can block it. Conditional compilation (%<COND ...>)
@@ --- selects Zork I/II/III-specific behavior from this shared codebase.
"Real Verb Functions"

@@ --- Miscellaneous responses (A-B): advent, alarm, answer, attack, back, blast
<ROUTINE V-ADVENT ()
...
@@ --- Movement and vehicle verbs: board, breathe, climb (up/down/on/foo), disembark, enter, exit, leave, leap, swim, walk
...
@@ --- Object state verbs: close, open, lock, unlock, turn, fill, deflate, inflate, plug
<ROUTINE V-CLOSE ()
...
@@ --- Object manipulation verbs: cut, burn, dig, drop, eat/drink, give, move, mung, pour, put, shake, take, throw, tie
<ROUTINE V-CUT ()
	 <COND (<FSET? ,PRSO ,ACTORBIT>
...
@@ --- Examination and information verbs: examine, find, listen, look (inside/behind/under/on), read, search, smell
<ROUTINE V-EXAMINE ()
...
@@ --- NPC interaction and communication: give, hello, command, tell, say, answer, reply, send, enchant/disenchant/incant
<ROUTINE PRE-GIVE ()
	 <COND (<NOT <HELD? ,PRSO>>
		<TELL 
...
@@ --- Light and fire verbs: lamp-on, lamp-off, strike (match), burn, melt
<ROUTINE V-LAMP-OFF ()
...
@@ --- Puzzle and magic words: odysseus, pray, treasure, exorcise, zork, frobozz, wish, win
<ROUTINE V-ODYSSEUS ()
	 <COND %<COND (<==? ,ZORK-NUMBER 1>
...
@@ --- Miscellaneous physical interactions (R-Z): rub, shake, skip, smell, spin, spray, squeeze, stab, stand, stay, strike, swing, wave, wear, wind, yell
...
@@ --- Verb-associated support routines: shared helpers called by verb handlers above.
@@ --- Organized into subsections: Descriptions, Scoring, Death, Object Manipulation, Miscellaneous.
"Verb-Associated Routines"

@@ --- Descriptions: room and object display routines (V-FIRST-LOOK, DESCRIBE-ROOM,
@@ --- DESCRIBE-OBJECTS, DESCRIBE-OBJECT, PRINT-CONTENTS, PRINT-CONT, FIRSTER, SEE-INSIDE?)
"Descriptions"
...
@@ --- Scoring: move counter, score globals, SCORE-UPD, SCORE-OBJ, won-flag trigger, YES? prompt
"Scoring"
...
@@ --- Death: globals for death state (DEAD, DEATHS, LUCKY); JIGS-UP and related routines are in ACTIONS.ZIL
"Death"
...
@@ --- Object manipulation helpers: ITAKE (pick up with weight/fumble checks), IDROP, CCOUNT, WEIGHT
"Object Manipulation"
...
@@ --- Miscellaneous: exit-type constants, indent table, HACK-HACK (generic action response),
@@ --- GOTO (room transition with vehicle/grue/lighting logic), navigation helpers (DO-WALK,
@@ --- GLOBAL-IN?, FIND-IN, HELD?, OTHER-SIDE, MUNG-ROOM, THIS-IS-IT), and random-response tables
"Miscellaneous"
