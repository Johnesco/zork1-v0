@@ PARSER.ZIL — Generic text parser for the Zork trilogy (started 7/28/83 by Marc Blank).
@@ Handles input tokenization (READ/LEXV), syntax matching against verb grammar tables,
@@ noun phrase resolution (scanning object trees by synonym/adjective), disambiguation
@@ (WHICH-PRINT prompts and orphaning for incomplete input), and pronoun tracking (P-IT-OBJECT).
@@ Globals follow a P- prefix convention. OOPS, AGAIN, and multi-sentence (THEN) support included.
...
@@ --- Global sentence-break characters and core action result globals (PRSA/PRSO/PRSI)
<SETG SIBREAKS ".,\"">
...
@@ --- Parser table pointers and clause-copy constants
<GLOBAL P-TABLE 0>
<GLOBAL P-ONEOBJ 0>
<GLOBAL P-SYNTAX 0>
...
@@ --- Core parser state: input length, direction, location, actor, and lexer tables
<GLOBAL P-LEN 0>
<GLOBAL P-DIR 0>
<GLOBAL HERE 0>
<GLOBAL WINNER 0>
...
@@ --- Continuation, pronoun, and orphan state globals
<GLOBAL P-CONT <>>
<GLOBAL P-IT-OBJECT <>>
...
@@ --- LEXV layout constants: word offsets, entry sizes, and part-of-speech bit positions
...
@@ --- Input/output parse tables (P-ITBL, P-OTBL) and verb table, plus noun-clause slot constants
...
@@ --- ROUTINE PARSER: Main entry point — reads input, handles OOPS/AGAIN, tokenizes words, identifies verb/direction/noun clauses, then dispatches to syntax check and object resolution
...
@@ --- ROUTINE STUFF: Copy contents of one LEXV table into another (used for AGAIN replay)
...
@@ --- ROUTINE INBUF-STUFF: Copy contents of one input buffer (INBUF) into another
...
@@ --- ROUTINE INBUF-ADD: Splice a corrected word from P-INBUF into OOPS-INBUF and fix AGAIN-LEXV pointers
...
@@ --- ROUTINE WT?: Test whether a word has a given part-of-speech bit set, optionally returning its value
...
@@ --- ROUTINE CLAUSE: Scan a noun clause in the input, recording its start/end pointers in P-ITBL
...
@@ --- ROUTINE NUMBER?: Parse a numeric literal (or time) from the input buffer, storing it in P-NUMBER
...
@@ --- ROUTINE ORPHAN-MERGE: Merge current input with a previous orphaned (incomplete) command
...
@@ --- ROUTINE ACLAUSE-WIN: Resolve an ambiguous clause by substituting the adjective into the orphaned noun clause
...
@@ --- ROUTINE NCLAUSE-WIN: Resolve disambiguation by copying the new noun clause into the orphaned slot
<ROUTINE NCLAUSE-WIN ()
...
@@ --- ROUTINE WORD-PRINT: Print a raw word from the input buffer given its length and starting byte offset
...
@@ --- ROUTINE UNKNOWN-WORD: Report an unrecognized word to the player and set up OOPS recovery
...
@@ --- ROUTINE CANT-USE: Report a known word used in an unrecognized grammatical context
...
@@ --- Syntax line layout constants and location-bit flags
<GLOBAL P-SLOCBITS 0>
...
@@ --- ROUTINE SYNTAX-CHECK: Match parsed verb/preps/noun-count against grammar syntax lines, orphaning on failure
...
@@ --- ROUTINE CANT-ORPHAN: Error message when a non-player actor cannot be prompted for disambiguation
<ROUTINE CANT-ORPHAN ()
	 <TELL "\"I don't understand! What are you referring to?\"" CR>
	 <RFALSE>>


@@ --- ROUTINE ORPHAN: Save current parse state into P-OTBL so incomplete input can be completed next turn
...
@@ --- ROUTINE THING-PRINT: Print the words of a noun clause (PRSO or PRSI) from P-ITBL pointers
...
@@ --- ROUTINE BUFFER-PRINT: Print a range of words from the LEXV buffer with optional "the" prefix
...
@@ --- ROUTINE PREP-PRINT: Print a preposition word given its internal prep ID
...
@@ --- ROUTINE CLAUSE-COPY: Copy a noun clause from one parse table to the orphan clause buffer
...
@@ --- ROUTINE CLAUSE-ADD: Append a single word to the orphan clause buffer (P-OCLAUSE)
...
@@ --- ROUTINE PREP-FIND: Look up a preposition ID in the PREPOSITIONS table and return its dictionary word
...
@@ --- ROUTINE SYNTAX-FOUND: Record the matched syntax line and set PRSA to its action
...
@@ --- ROUTINE GWIM: "Get What I Mean" — attempt to infer a missing noun from context using GWIMBIT flags
...
@@ --- ROUTINE SNARF-OBJECTS: Resolve noun clauses (NC1/NC2) into object lists in P-PRSO/P-PRSI, handling BUT/EXCEPT
...
@@ --- ROUTINE BUT-MERGE: Remove BUT/EXCEPT objects from a match table, returning the filtered table
...
@@ --- Object-matching globals: current noun/adjective, match tables (PRSO/PRSI/BUTS/MERGE), and ALL/ONE flags
<GLOBAL P-NAM <>>
<GLOBAL P-ADJ <>>
<GLOBAL P-ADVERB <>>
<GLOBAL P-ADJN <>>
...
@@ --- ROUTINE SNARFEM: Walk a noun clause word-by-word, setting P-NAM/P-ADJ and calling GET-OBJECT for each noun group
...
@@ --- Search-location bit constants for DO-SL (self, held, contained, on-ground, etc.)
<CONSTANT SH 128>
<CONSTANT SC 64>
<CONSTANT SIR 32>
<CONSTANT SOG 16>
...
@@ --- ROUTINE GET-OBJECT: Find matching objects by name/adjective/GWIMBIT, searching local then global scope
...
@@ --- ROUTINE WHICH-PRINT: Print the "Which X do you mean..." disambiguation prompt listing matching objects
...
@@ --- ROUTINE GLOBAL-CHECK: Search global objects, pseudo-objects, and GLOBAL-OBJECTS for noun matches
...
@@ --- ROUTINE DO-SL: Dispatch SEARCH-LIST with top/bottom/all scope based on location bits
...
@@ --- ROUTINE SEARCH-LIST: Recursively search an object's children for noun/adjective matches, respecting container visibility
...
@@ --- ROUTINE OBJ-FOUND: Append a matched object to the result table
<ROUTINE OBJ-FOUND (OBJ TBL "AUX" PTR)
	<SET PTR <GET .TBL ,P-MATCHLEN>>
	<PUT .TBL <+ .PTR 1> .OBJ>
	<PUT .TBL ,P-MATCHLEN <+ .PTR 1>>>

@@ --- ROUTINE TAKE-CHECK: Verify PRSO/PRSI are held if syntax requires SHAVE/STAKE, auto-taking if needed
<ROUTINE TAKE-CHECK ()
	<AND <ITAKE-CHECK ,P-PRSO <GETB ,P-SYNTAX ,P-SLOC1>>
	     <ITAKE-CHECK ,P-PRSI <GETB ,P-SYNTAX ,P-SLOC2>>>>

@@ --- ROUTINE ITAKE-CHECK: For a single object table, attempt implicit take or report "you don't have that"
...
@@ --- ROUTINE MANY-CHECK: Reject multiple direct/indirect objects when the syntax does not allow SMANY
...
@@ --- ROUTINE ZMEMQ: Search a word table for an item, returning a pointer to the match or false
...
@@ --- ROUTINE ZMEMQB: Search a byte table for an item, returning true/false
...
@@ --- ROUTINE LIT?: Test whether a room is lit (inherently or by a light source carried by the player/NPC)
...
@@ --- ROUTINE THIS-IT?: Test whether an object matches the current P-NAM/P-ADJ/P-GWIMBIT search criteria
...
@@ --- ROUTINE ACCESSIBLE?: Test whether the player can physically touch an object (reachability through open containers)
...
@@ --- ROUTINE META-LOC: Walk the object tree upward to find the top-level room containing an object
